<?php
/**
 * hBootstrap : Twitter Bootstrap Helper
 *
...
class hBootstrap
{
	 ...
	 * @var string
	 */
	static public $form_helper = 'hForm';
	...
	public static function form(Array $args)
	{
		...
		if ($type === 'multiple') {
			...
			foreach ($args['multiple'] as $values){
				...
				$controls.= call_user_func_array(array(static::$form_helper, $type), $values);
			}
		...
		// single element
			...
			$values   = static::normalize_array($args[$type]);
			$controls = call_user_func_array(array(static::$form_helper, $type), $values);
			$for	  = $values[0];
		...
		$dom.= call_user_func_array(array(static::$form_helper, 'label'), array(null, $label, $for, 'control-label'));
	 ...
	 * @return array		out
	 */
	private static function normalize_array(Array $array){
		$new_array   = array();
	 ...
	 */
	public static function nav($nav_class, Array $items, $url_filter = null, $url_active = null)
	{
		$dom = null;
		...
		return ($nav_class) ? '<ul class="nav '.$nav_class.'">'.$dom.'</ul>' : $dom;
	}
	 ...
	 */
	static public function form_title($label)
	{
		return '
